...
final class DatabaseService: ObservableObject {
    ...
    func createTask(roomId: UUID, isGeneralHouseholdTask: Bool = false, assignedMembershipId: UUID? = nil, title: String, frequencyDays: Int, priority: Priority, estimatedMinutes: Int) async throws -> Task { let effectiveRoomId = isGeneralHouseholdTask ? Task.generalHouseholdRoomId : roomId; let next = Self.dateOnlyFormatter.date(from: Self.addDays(Self.todayISO(), days: frequencyDays / 2)) ?? Date(); let existing = try context.fetch(FetchDescriptor<StoredTask>()).filter { $0.roomId == effectiveRoomId && $0.isGeneralHouseholdTask == isGeneralHouseholdTask }; let value = StoredTask(roomId: effectiveRoomId, isGeneralHouseholdTask: isGeneralHouseholdTask, assignedMembershipId: assignedMembershipId, title: title, frequencyDays: frequencyDays, priority: priority, estimatedMinutes: estimatedMinutes, nextDueAt: next, sortOrder: (existing.map(\.sortOrder).max() ?? -1) + 1); context.insert(value); try context.save(); publishWidgetSnapshot(); return Self.task(value) }
    func updateTask(id: UUID, title: String?, frequencyDays: Int?, priority: Priority?, estimatedMinutes: Int?, roomId: UUID?, isGeneralHouseholdTask: Bool? = nil, assignedMembershipId: UUID? = nil, updateAssignment: Bool = false) async throws { guard let value = try storedTask(id) else { return }; if let title { value.title = title }; if let frequencyDays { value.frequencyDays = frequencyDays }; if let priority { value.priority = priority.rawValue }; if let estimatedMinutes { value.estimatedMinutes = estimatedMinutes }; if let isGeneralHouseholdTask { value.isGeneralHouseholdTask = isGeneralHouseholdTask; value.roomId = isGeneralHouseholdTask ? Task.generalHouseholdRoomId : (roomId ?? value.roomId) } else if let roomId { value.roomId = roomId }; if updateAssignment { value.assignedMembershipId = assignedMembershipId }; value.updatedAt = Date(); try context.save(); publishWidgetSnapshot() }
    ...
    func applySupabaseSnapshot(
        ...
        for remote in remoteTasks {
            ...
            if let local = localTasks[remote.id] {
                ...
                local.assignedMembershipId = remote.assignedMembershipId
                ...
                context.insert(StoredTask(
                    ...
                    assignedMembershipId: remote.assignedMembershipId,
                    title: remote.title,
